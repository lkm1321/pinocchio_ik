from setuptools import setup, find_packages
import os
from glob import glob

package_name = 'pinocchio_ik'

setup(
    name=package_name,
    version='0.0.1',
    packages=find_packages(exclude=['test']),
    data_files=[
        ('share/ament_index/resource_index/packages',
            ['resource/' + package_name]),
        ('share/' + package_name, ['package.xml']),
        (os.path.join('share', package_name, 'launch'), glob('launch/*')),
        (os.path.join('share', package_name, 'capnp'), glob('capnp/*.capnp')),
    ],
    install_requires=['setuptools'],
    zip_safe=True,
    maintainer='Brian Lee',
    maintainer_email='brian@erl-brian.ucsd.edu',
    description='Pinocchio-based velocity IK and FK CBF for collision avoidance.',
    license='Apache License 2.0',
    entry_points={
        'console_scripts': [
            'velocity_ik = pinocchio_ik.velocity_ik:main',
            'distance_cbf = pinocchio_ik.cbf_node:main',
            'distance_cbf_service = pinocchio_ik.distance_cbf_service:main',
            'distance_cbf_capnp = pinocchio_ik.distance_cbf_capnp:main',
            'vr_twist_publisher = pinocchio_ik.vr_twist_publisher:main',
            'canned_twist = pinocchio_ik.canned_twist:main',
            'fov_demo = pinocchio_ik.fov_demo:main',
            'image_recorder = pinocchio_ik.image_recorder:main',
        ],
    },
)
